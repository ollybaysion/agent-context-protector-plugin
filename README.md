...
Phase 1 (`input-gate` + `output-cap`) and Phase 2 (`ctx-budget`, `analyze`)
are active; the rest are planned per phase.
...
| `model-guard` (model-advisor) | Stop | ✅ active | Always-on statusline verdict on whether the current model fits recent work: `모델 적합(작업형/대화형)` when fine, `⚠ /model sonnet 권장(…)` only in the one flaggable cell — an expensive model (Fable/Opus) on a conversational streak. Budget-only (downgrade direction; judging a cheap model under-powered is analyze model-fit's job, #22); advice-only — no hook can switch the main model — and self-erases once the live model moves on. **Requires `statusLine` configured** (see [core/model-guard/DESIGN.md](core/model-guard/DESIGN.md)); `subagent-guard` (forced subagent downgrade) is designed but introduction is still undecided |
| `analyze` | — (CLI + user-invoked skill, not a hook) | ✅ active | Offline diagnostics over transcripts. Default report needs **no plugin install** (`npx github:ollybaysion/agent-context-protector-plugin`): usage/$ totals, pattern-family spend, rule proposals, `--precise`. `--plugin-report` adds hook-effect sections: cap/deny ledgers, per-module $ savings (measured remaining-turn multiplier), inefficiency diagnostics. In-session: `/agent-context-protector:analyze` |
