...
| `model-guard` (model-advisor) | Stop | ✅ active | Flags a main session running an expensive model (Fable/Opus) through a conversational-only streak by writing an advisory the statusline HUD renders as an always-on segment (e.g. `/model sonnet 권장(8중 6턴 대화형)`); advice-only — no hook can switch the main model — and self-erases once the live model moves on. **Requires `statusLine` configured** (see [core/model-guard/DESIGN.md](core/model-guard/DESIGN.md)); `subagent-guard` (forced subagent downgrade) is designed but introduction is still undecided |
