---
name: analyze
description: 토큰 지출 진단 리포트 — 트랜스크립트를 오프라인 분석해 패턴별 지출, output-cap/deny 원장, 룰 제안을 요약한다. 사용자가 "/analyze", "토큰 분석", "지출 리포트", "뭐가 토큰 먹었지" 등으로 명시적으로 요청할 때만 발동. 인자 [--project <dir> | all] [--since <ISO>] [--top N] [--precise] [--json <path>]는 CLI로 그대로 전달.
---
