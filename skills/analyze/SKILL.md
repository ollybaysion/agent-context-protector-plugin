---
name: analyze
description: 토큰 지출 진단 리포트 — 트랜스크립트를 오프라인 분석해 전체 비용, 패턴별 지출, 룰 제안을 요약한다. 사용자가 "/analyze", "토큰 분석", "지출 리포트", "뭐가 토큰 먹었지" 등으로 명시적으로 요청할 때만 발동. 훅 절감 효과·비효율을 물으면 --plugin-report를, 넛지 순응률을 물으면 --nudge-report(대시보드 반영까지 원하면 --push-outcomes)를 추가한다. 인자 [--project <dir> | all] [--since <ISO>] [--top N] [--precise] [--plugin-report] [--nudge-report] [--push-outcomes] [--json <path>]는 CLI로 그대로 전달.
---
...
3. **실행**: `node <CLI 경로> --project <dir> <나머지 인자>` — `--top`은
   ...
   **`--nudge-report`는 넛지 순응률을 물을 때만**("넛지 먹혔어?", "순응률") —
   넛지 원장은 전역이므로 이때는 `--project` 없이(all) 실행해야 트랜스크립트
   매칭이 안 새는다. 대시보드 Nudges 탭까지 갱신해 달라면 `--push-outcomes`.
...
4. **해석해서 보고** (원문 통째 덤프 금지 — 리포트는 해석이 가치다):
   ...
   - `--nudge-report`를 썼다면: complied/judged와 기저율 차감 후 순응률,
     verdict(표본 부족 포함)를 전달한다. keep-audit misassigned가 1 이상이면
     수명 규칙 결함 신호이므로 반드시 강조한다(이슈 #21 트립와이어).
