...
**판정 — 상시 verdict (예산 전용, 결정적):**

```text
가벼운 턴 = 툴 호출 ≤ 1  AND  무거운 도구 없음
작업 모드 = (가벼운 턴 ≥ threshold(기본 6)) ? 대화형 : 작업형
verdict   = (현재모델 ∈ expensive AND 대화형) ? "⚠ /model <target> 권장(…)"
                                              : "모델 적합(<모드>)"
턴 부족(윈도 미달 — 세션 초반·컴팩션 직후) → 판정 불가 → 무표시(클리어)
```

판정 매트릭스 — 권고가 뜨는 칸은 "비싼 모델 × 대화형" 하나뿐, 나머지는 전부
`적합`. 업그레이드 방향(싼 모델 under-power)은 범위 밖(§8-h, analyze
model-fit #22).
...
```json
{ "modelAdvice": {
    "text": "⚠ /model sonnet 권장(대화형 8중 6)",
    "model": "claude-fable-5",
    "ts": 1751696000000 } }
```

- 판정 불가(윈도 미달)·모듈 비활성일 때만 `{}` 저장(무표시). 그 외엔 **항상**
  `적합` 또는 `권장` verdict를 쓴다 — statusline이 상시 렌더한다.
- `text`는 훅에서 미리 살균(제어문자 제거)한 완성 문자열 — statusline은
  조립하지 않는다.
- `model`은 트랜스크립트에서 읽은 정확한 모델 ID — statusline 자기 소거의 비교 키.

**매 Stop 재평가가 핵심 메커니즘**: verdict는 항상 "마지막 턴 기준 최신 판정"
이고, 대화형↔작업형 전환·모델 전환이 다음 Stop에 그대로 반영된다. 쿨다운·
세션당 1회 개념이 필요 없는 이유다.
...
- **(h) 싼 모델의 `적합`은 "예산상 적합"일 뿐이다.** 이 모듈은 싼 모델이
  작업에 비해 약한지(under-power)를 판정하지 않는다 — 업그레이드는 범위 밖
  (analyze model-fit #22). 그래서 sonnet/haiku에는 작업 종류와 무관하게 늘
  `적합`이 뜬다(예산 관점에선 정확하다: 싼 모델 = 낭비 아님).
