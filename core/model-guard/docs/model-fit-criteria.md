...
넓히면 무거운 오발화 34→8건·7→2세션, 잡담은 여전히 발화(11/34). 미결 결정 하나가 남아
있다(§9, `keepOnThinking`, 기본 OFF).
...
- **비싼 모델로 순수 텍스트 긴 설계 토론**은 다운그레이드 넛지를 받는다(§9). 결정적 판별
  신호(thinking 본문 길이)가 트랜스크립트에서 삭제돼 차단 불가. 확대 게이트 적용 후 남는
  무거운 발화 8건이 이 케이스다.
- **비싼 모델로 긴 읽기 전용 조사**(`rg`/`cat`/Read 다수, 편집 없음)는 작업형(`hasTool`)
  → 넛지 없음. 의도적 보수(과발화 시 유저가 기능을 꺼버림, DESIGN §5).
- **발화 산발성**: 정작 잡을 잡담 세션(`f0cf3e00`)에서도 다운그레이드는 **약 42% Stop에서만
  발화**(58%는 침묵)한다 — 장문 잡담의 256KB 꼬리가 자주 `chatMinTurns=8` 논리턴 미만이라
  판정 불가로 떨어지기 때문. 안전 방향(거짓 넛지 아님)이지만 말할 곳에서 자주 침묵한다.
- **한 턴 지연**(DESIGN §8-f) 불변: verdict는 마지막 Stop 기준.
- **읽기 전용 `git`/`gh`가 작업으로 분류**되는 비대칭(§3) — 넛지 억제 방향이라 수용, 튜닝
  여지 있음.
...
- **권고: 수용**(문서 저자 판단). (1) 결정적 판별 신호가 없고(persist된 신호 중 판별자
  없음), (2) 순수 텍스트 설계 토론은 실제로 sonnet으로 충분한 경우가 많아 "틀린 넛지"라
  단정 어렵고, (3) 강제 아닌 상시 세그먼트 한 줄이라 무시 비용이 0에 가깝다.
  `keepOnThinking`은 옵트인 노브로만 유지(extended-thinking을 안 쓰는 유저가 §8-d 보호를
  원하면 켤 수 있게).
